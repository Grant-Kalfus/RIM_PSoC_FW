...
void set_param(byte param, uint value) 
{
  ...
  param_handler(param, value);
}

uint get_param(byte param)
{
  transfer(GET_PARAM | param);
  return param_handler(param, 0);
}
...
{
  transfer(MOVE | dir);
  transfer((byte)(n_step >> 16));
  transfer((byte)(n_step >> 8));
  transfer((byte)(n_step));
}
...
uint param_handler(byte param, uint value)
{
  ...
  switch (param)
  {
    ...
    case ACC: 
      ret_val = send_and_receive(value, 12);
      break;
    case DECEL: 
      ret_val = send_and_receive(value, 12);
      break;
    ...
    //  for this register.
    case MAX_SPEED:
      ret_val = send_and_receive(value, 10);
      break;
    ...
    //  well enough.
    case INT_SPD:
      ret_val = send_and_receive(value, 14);
      break;
    case ST_SLP: 
      ret_val = transfer((byte)value);
      break;
    ...
    case K_THERM: 
      ret_val = transfer((byte)value & 0x0F);
      break;
    ...
    case STALL_TH: 
      ret_val = transfer((byte)value & 0x7F);
      break;
    ...
    case CONFIG: 
      ret_val = send_and_receive(value, 16);
      break;
...
uint send_and_receive(uint value, byte bit_len) {
    ...
    if (byte_len == 3) {
        r |= transfer((byte)(value>>16)) << 16;
    }
    if (byte_len >= 2) {
        r |= transfer((byte)(value>>8)) << 8;
    }
    if (byte_len >= 1) {
        r |= transfer((byte)value);
  }
    return (r & mask);
}
